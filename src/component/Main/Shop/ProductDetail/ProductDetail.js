...
class ProductDetail extends Component {
  ...
  onAddToCart = (product) => {
    ...
    product ={...product, qty: this.state.qty}
    this.props.onAddToCart(product);
  };
  componentWillMount() {
    this.props.actGetTTShop(this.props.route.params.product.shop_id);
  }
  onUpdateQuantity = () => {
    var a = this.state.qty + 1;
    this.setState({qty:a});
  };
  ...
  render() {
    const { route, myshop ,navigation} = this.props;
            ...
            <View style={footer}>
              <View style={titleContainer}>
                ...
                <Text style={txtMaterial}>
                  Giảm giá: {product.phan_tram_km} %
                </Text>
              </View>
              <TouchableOpacity
                onPress={() => {
              ...
              >
                <Text style={titleShop}>
                  Tên shop: {myshop.inforShopcard.tenshop} | địa chỉ{" "}
                  {myshop.inforShopcard.dia_chi}
                </Text>
              </TouchableOpacity>
              <View style={numberOfProduct}>
                        <TouchableOpacity
                          onPress={() => this.onDownQuantity()}
                        >
                          <AntDesign name="minus" color="black" size={30} />
                        </TouchableOpacity>
                        <Text style={{ fontSize: 20 }}>SL: {this.state.qty}</Text>
                        <TouchableOpacity
              ...
              <View style={descContainer}>
                ...
                <Text style={txtMaterial}>*****Đánh giá</Text>
                <Star />
                <HTML
...
const mapDispatchToProps = (dispatch) => {
  return {
    onAddToCart: (product, token) => dispatch(actAddToCart(product, token)),
